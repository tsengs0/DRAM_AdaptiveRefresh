...
for(int iter = 0; iter < CheckNum*4; iter++) {
	...
	x_prime[sub_refresh_id] = floor(A_t/L_req)*BI; 
	if(calc_tRFC_ap(x_prime[sub_refresh_id]) <= bound_t) {
		x_prime[sub_refresh_id] += BI;
		while(calc_tRFC_ap(x_prime[sub_refresh_id]) <= bound_t) x_prime[sub_refresh_id] += BI;
	}
	else {
		while(calc_tRFC_ap(x_prime[sub_refresh_id]) > bound_t) x_prime[sub_refresh_id] -= BI;
	}
	UpperBound = calc_tRFC_ap(x_prime[sub_refresh_id]);
	
	refresh_percent = (
			(double) calc_tRFC_ap(i)*CheckNum 
		)*100/(double) tREFW;
	prev_access_rows[(iter+(CheckNum-1))%CheckNum] = x_prime[sub_refresh_id];
	if(sub_refresh_id == 0) {
		//cout << "--------------------------------------------------------------------" << endl;
		//cout << "Refresh Windows_" << (iter/CheckNum) << endl;
	}
	//cout <<  i << "/" << x_prime[sub_refresh_id] << " (rows): " << (calc_tRFC_ap(i)*100)/sub_refresh_cycle << "%" << endl;
 ...
 //-----------------------------------------------------------------------------
	...
	for(int j = 0; j < CheckNum; j++) temp_1 += floor((float) x_prime[j] / (float) BI);
